...
\begin{definition}
  If $\phi \in \mathcal{L} (\Phi)$ then let $\phi^{\ast}$ be the same
...
& \DP\DP\DP\DP\DP\DP\DP\DP\DP\DP\DP\DP\DP\DP\DP\DP\DP\DP\DP\DP\bot & 
\IFF \Box^{70}\bot
\end{eqnarray*}
\end{example}
...
Further, note that $\mathcal{L}_0 (\Phi) \subseteq \mathcal{L}_A (\Phi) \cap \mathcal{L}_B
